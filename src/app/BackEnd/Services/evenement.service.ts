...
export class EvenementService {
  ...
  DeleteEvenement(id:number){
      return this.http.delete(this.BackEndURL+"DeleteEvent/"+id,{ ...options, responseType: 'text' }).pipe(
        map(
          userData => {
	...
	UpdateEvenement(event:Event){
       return this.http.put(this.BackEndURL+"UpdateEvent",event,{ ...options, responseType: 'text' }).pipe(
          map(
            userData => {
            }
          )
