...
export class ShowOfferComponent implements OnInit {
  ...
  constructor(private s: OfferService, private activate: ActivatedRoute, private router: Router) {}
  ...
  getById() {
    ...
    this.s.showById(this.id).subscribe((d) => {
      this.offer = d;
    });
  }
  ...
  allSimilarOffers() {
    let idUser = 1;
    this.id = this.activate.snapshot.params['id'];
    this.s.findSimilarOffers(this.id, idUser).subscribe((e) => {
